...
public class ValidatingMultipleWindowTitles {
	WebDriver driver;
	SoftAssert sf;
	WebDriverWait wait;
	...
	public void validatingPurchase() throws InterruptedException {
		...
		String fbHanlde = null;
		for (String handle : allHandles) {
			if ((!handle.equals(parentHandle)) || (!handle.equals(googleHanlde))) {
				fbHanlde = handle;
		...
		allHandles = driver.getWindowHandles();
		String yahooHanlde = null;
		for (String handle : allHandles) {
			if ((!handle.equals(parentHandle)) || (!handle.equals(googleHanlde)) || (!handle.equals(fbHanlde))) {
				yahooHanlde = handle;
